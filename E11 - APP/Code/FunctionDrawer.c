 ...
 *
 */
void save(GtkApplication *app, gpointer user_data) {
	/*
	 * Detect the current window, to use as parent for the file chooser window
	 */
	GtkWidget *dialog;
	...
	if (gtk_dialog_run(GTK_DIALOG (dialog) ) == GTK_RESPONSE_ACCEPT) {
		...
		if (file != NULL ) {
			 ...
			 * using fprintf, save the whole content of
			 * "filtered"
			 * within a text file. The format must be:
			 * "%lf %lf\n"
			 */
			printf("Whooops... It looks like I forgot to implement this. You will have to do it yourself.\n");
			/*
			 * Close the file
			 */
			fclose(file);
...
static void activate(GtkApplication *app, gpointer user_data) {
	 ...
	 */
	GtkWidget *hlayout, *rlayout, *minx_scale, *maxx_scale, *miny_scale, *maxy_scale, *precision_scale;
	/*
